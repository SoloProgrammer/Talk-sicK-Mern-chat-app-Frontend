...
function MessageBox() {
  ...
  useEffect(() => {
    ...
    if (messageText.length > 0)
      socket.emit("typing", selectedChatCompare?._id, user);
  ...
  const getSelectedChatDownloadedMsgs = () => {
    ...
    return chatMessagesCompare?.filter(
      (ch) => ch.chatId === selectedChatCompare?._id
    )[0]?.messages;
  };
    ...
    chatMessagesCompare.forEach((cmp) => {
      ...
      if (cmp.chatId === newMessage?.chat?._id) {
        let chatWithNewMsgs = {
          ...
          messages: [...cmp.messages, newMessage],
        };
        ...
        setChatMessages([
          ...chatMessagesCompare.filter(
            (chm) => chm.chatId !== selectedChatCompare._id
          ),
          chatWithNewMsgs,
        ]);
      }
  ...
  function updateChats(newMessageRecieved) {
    let updatedChats;
    if (chatsCompare) {
      updatedChats = chatsCompare?.map((c) => {
        if (c._id === newMessageRecieved.chat._id) {
          c.leftFromGroup = c.leftFromGroup.filter(
            (LfgObj) => LfgObj.user._id !== user?._id
          );
          selectedChatCompare &&
            selectedChatCompare._id === c._id &&
            setSelectedChat(c);
        }
        return c;
      });
  ...
  useEffect(() => {
    const messageReceivedEventListener = async (newMessageRecieved, User) => {
      ...
      if (messageEmitterCount === 0) {
        let chatMsgs = getSelectedChatDownloadedMsgs();

        if (!messageEmitterCount) messageEmitterCount += 1;
        ...
        // console.log(messageEmitterCount, selectedChatCompare, newMessageRecieved, newMessageRecieved.content.message.includes(`added ${User.name.split(' ')[0]}`), `added ${User.name.split(' ')[0]}`);
        if (
          selectedChatCompare?._id === newMessageRecieved.chat._id &&
          newMessageRecieved.content.message.includes(
            `added ${User.name.split(" ")[0]}`
          )
        ) {
          updateChats(newMessageRecieved);
          let SelectedChatClone = structuredClone(selectedChatCompare);
          SelectedChatClone.leftFromGroup =
            newMessageRecieved.chat.leftFromGroup;
          SelectedChatClone.totalMessages =
            newMessageRecieved.chat.totalMessages;
          setSelectedChat(SelectedChatClone);
          setChatMessages(
            chatMessagesCompare?.filter(
              (chm) => chm.chatId !== newMessageRecieved?.chat?._id
            )
          );
          setIsFetchMessagesAgain(true);
          setTimeout(() => {
            messageEmitterCount = 0;
          }, 200);
          return;
        }
        ...
        if (
          ...
          if (!socket) return;
            ...
            if (
              notificationsCompare.length === 0 ||
              (notificationsCompare.length > 0 &&
                !notificationsCompare
                  .map((noti) => noti.chat._id)
                  .includes(newMessageRecieved.chat._id))
            ) {
              if (
                !newMessageRecieved.chat.archivedBy.includes(User._id) &&
                !newMessageRecieved.chat.mutedNotificationBy.includes(User?._id)
              ) {
  ...
  const sendMessage = async () => {
    if (messageText.replace(regx, "").length === 0 && !sendPic)
    ...
    try {
      ...
      if (
        ...
        !chatMessages.map((ch) => ch.chatId).includes(selectedChat._id)
      ) {
        ...
        chatMessages.forEach((chatMsg) => {
          if (chatMsg.chatId === selectedChat?._id) {
            chatMsg = {
              chatId: selectedChat._id,
              messages: [...chatMsg.messages, json.fullmessage],
            };
  ...
  return (
    <Box
      ...
      display={{
        ...
        md: selectedChat ? "flex" : "none",
        lg: "flex",
      }}
      className="messagesBox"
      width={{ base: "100%", md: "100%", lg: "64%" }}
    >
      {!selectedChat ? (
        <Box
          zIndex={1}
        ...
        >
          <BrandLogo />
          <Image
            className="chat1_icon"
            opacity={".7"}
            width="220px"
            src={chat2_icon}
          />
          <Image
            className="chat2_icon"
            opacity={".7"}
            width="220px"
            src={chat1_icon}
          />
          <Box className="messagesBoxText">
            <Text
              ...
              fontSize={{ md: ".9rem" }}
            >
              Talk-sicK a Chat app Project with live personal as well as group
              messaging functionality
              <br />
              You will also recieve live Notifications from chats you have
              created for the newly recieved messages
            </Text>
            <br />
            <Text
              fontWeight={"medium"}
              ...
              textAlign="center"
            >
              Search and Create a Chat or Click on the Existing Chat to Start
              Conversation..!
            </Text>
          </Box>
        </Box>
        ...
        >
          ...
          <MessagesBox
            ...
            isFirstLoadOfMsgs={isFirstLoadOfMsgs}
            setIsFirstLoadOfMsgs={setIsFirstLoadOfMsgs}
          />
          ...
          >
            <Box
              ...
              className={`messageInputBox ${
                isUserRemovedFromChat(selectedChat, user) && "disabled"
              }`}
            >
              {isEmojiPick && (
                <Box
                  width={{ base: "315px", md: "360px" }}
                  pos={"absolute"}
                  bottom="6.5rem"
              ...
              >
                <Linkify>
                  <input
                    ...
                    placeholder={
                      sendPic
                        ? "Type your message OR Send Image...."
                        : "Type your message......"
                    }
                    id="text"
                    type="text"
                ...
                <Box className="flex" gap={"1rem"}>
                  ...
                  <SendImageModal isOpen={isOpen} onClose={onClose}>
                    <Tooltip placement="top" fontSize={".8rem"} label="Image">
                      <Image
                        onClick={onOpen}
                        cursor={"pointer"}
                        src="https://cdn-icons-png.flaticon.com/512/4303/4303472.png"
                        opacity={".5"}
                        width={"1.7rem"}
                      />
                    </Tooltip>
                  ...
                  {sendPic && (
                    ...
                    >
                      ...
                      >
                        <i
                          className="fa-solid fa-xmark sendPicx-mark"
                          onClick={() => setSendPic(null)}
                        />
                      </Tooltip>
                      <Text
                        fontSize={".95rem"}
                  ...
                  {loading ? (
                    ...
                    >
                      <Image
                        ...
                        onClick={sendMessage}
                        className={`${
                          messageText.replace(regx, "").length > 0 && "sentBtn"
                        }`}
                        src={`${
                          messageText.replace(regx, "").length !== 0 || sendPic
                            ? sendBtnActive
                            : sendBtn
                        }`}
                      />
