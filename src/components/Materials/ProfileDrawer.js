...
function ProfileDrawer({ width, align = "right" }) {
  ...
  const HandleDetailChange = (e) => {
    ...
    setProfileDetail({
      ...
      [e.target.name]:
        e.target.name === "phone"
          ? Number(String(e.target.value).slice(0, 10))
          : e.target.value,
    });
  };
  ...
  const HandleDetailSave = () => {
    ...
    Object.keys(profileDetail).forEach((key) => {
      if (profile.isGrpProfile) {
        if (profileDetail["name"].replace(regx, "") !== profile["name"])
          isChanged = true;
      } else {
        if (key !== "phone") {
          if (profileDetail[key].replace(regx, "") !== profile[key])
            isChanged = true;
        } else {
    ...
    if (isChanged) {
      ...
      if (isProcessable) {
        if (profileDetail.phone && String(profileDetail.phone).length < 10) {
          return showToast(
            "Invalid",
            "Phone cannot have less than 10 digits",
            "error",
            3000
          );
        }
        ...
        profile?._id === user?._id &&
          setProfileDetail({
            ...profileDetail,
            about: profileDetail.about.replace(regx, " "),
          });
  ...
  return (
    <Box
      className={`profileDrawer ${
        align === "right"
          ? "right0 translateXFull maxWidth520"
          : "left0 translateXFull-"
      }`}
      width={"full"}
      pos={"absolute"}
      transition="all .3s"
      ...
      height={`${
        window.innerWidth > 770
          ? `calc(100vh - ${profile?._id === user?._id ? "11rem" : "10.1rem"})`
          : `calc(100vh - ${profile?._id === user?._id ? "11rem" : "9.6rem"})`
      }`}
      ...
      >
        {/* confirmBox modal for Removing profileAvatar */}
        {
          <RemoveAvatarConfirmModal
            ...
            confirm={confirm}
            handleRemoveProfileAvatar={handleRemoveProfileAvatar}
            user={user}
            profile={profile}
          />
        }
            ...
            {!saved ? (
              ...
              <Tooltip label="save" placement="bottom">
                <Image
                  ...
                  cursor="pointer"
                  src="https://cdn-icons-png.flaticon.com/512/1293/1293029.png"
                  width={"2rem"}
                />
              </Tooltip>
            )}
              ...
              <Avatar
                ...
                onClick={(e) => {
                  ...
                  !profile.isGrpProfile
                    ...
                    ? !isedit &&
                      setProfilePhoto(getSender(selectedChat, user)?.avatar)
                ...
                cursor={"pointer"}
                src={profile?.avatar || defaultPic}
                width="11rem"
                height={"11rem"}
                position="relative"
              >
                {(profile._id === user?._id ||
                  (profile?.isGrpProfile && isAdmin(selectedChat, user))) &&
                  profile.avatar !== defaultPic &&
                  profile.avatar !== defaultGrpPic && (
                    <Tooltip
                      label="Remove avatar"
                      placement="top"
                      fontSize={".75rem"}
                    >
                      <Box
                        ...
                        background="white"
                        borderRadius={"50%"}
                        boxShadow="inset 0 0 1.5px rgba(0,0,0,.3)"
                      >
                        <Image
                          src="https://cdn-icons-png.flaticon.com/512/5165/5165608.png"
                          width={"1.1rem"}
                        />
                      </Box>
                    </Tooltip>
              ...
              <Box className="InptBox flex nameInptBox" gap={".5rem"}>
                {isedit ? (
                  <input
                    ...
                    disabled={
                      !selectedChat?.isGroupchat && profile?._id !== user?._id
                    }
                  ...
                  <Text textAlign={"center"}>{profileDetail.name}</Text>
                )}
              </Box>
              ...
              >
                {isedit ? (
                  <textarea
                    ...
                    style={{
                      width: profileDetail.about.length + "ch",
                      textAlign: "center",
                    }}
                    maxLength="90"
                    rows={1}
              ...
              >
                ...
                <Box width={"full"}>
                  <Box>
                    ...
                    <Box
                      className={`flex ${
                        profile?._id === user?._id ? "disabledBg" : "InptBox"
                      }`}
                      gap={".5rem"}
                      padding=".9rem 0"
                      width="100%"
                  ...
                  <Box marginTop={".4rem"}>
                    <Text fontWeight={"hairline"} fontSize=".9rem">
                      Phone
                    </Text>
                    <Box
                      className="InptBox flex"
        ...
        {profile?._id === user?._id && (
          <Box
            padding={{ base: "0rem .8rem", md: "0 1.1rem" }}
            width="full"
            marginTop={".5rem"}
          >
        ...
        {selectedChat?.isGroupchat && user?._id !== profile?._id && (
          <Box
            className="flex"
            width={"100%"}
            padding={{ base: "0 .5rem", md: "0 1rem" }}
            paddingRight={{ base: ".15rem", md: ".5rem" }}
          >
            <GroupMembersBox />
          </Box>
        )}
        ...
        {profile?._id !== user?._id && !profile?.isGrpProfile && (
          <Box
            width={"100%"}
            marginTop={".5rem"}
            padding={{ base: "0 .8rem", md: "0 1.1rem" }}
            display="flex"
            flexDir={"column"}
            gap=".2rem"
          >
        ...
        {profile?._id !== user?._id &&
          ...
          !isUserRemovedFromChat(selectedChat, user) && (
            <ConfirmBoxModal
              handleFunc={() =>
                handleLeaveGrp(selectedChat, onClose, setLoading)
              }
              isOpen={isOpen}
              onClose={onClose}
              modalDetail={{
              ...
              <Box padding={{ base: "0 .8rem", md: "0 1.1rem" }} width={"100%"}>
                ...
                >
                  ...
                  >
                    <Image
                      width="1.1rem"
                      src="https://cdn-icons-png.flaticon.com/512/8914/8914318.png"
                    />
                    Leave Group
                  </Text>
                </Box>
        ...
        {profile?._id !== user?._id &&
          !archivedChats.map((c) => c._id).includes(selectedChat?._id) && (
            <Box padding={{ base: "0 .8rem", md: "0 1.1rem" }} width={"100%"}>
              <Box
                  ...
                  <Image
                    ...
                    src={`${
                      selectedChat?.pinnedBy?.includes(user?._id)
                        ? "https://cdn-icons-png.flaticon.com/512/1274/1274749.png"
                        : "https://cdn-icons-png.flaticon.com/512/1274/1274786.png"
                    }`}
                  />
                  {selectedChat?.pinnedBy.includes(user?._id)
                    ? "Unpin chat"
